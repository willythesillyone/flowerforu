...
function createPlane() {
    shaderMaterial = new THREE.ShaderMaterial({
        uniforms: {
            ...
            u_fade: { type: "f", value: 0.5 } // Tambahan untuk fading
        },
...
function render() {
    ...
    if (pointer.clicked) {
        ...
        shaderMaterial.uniforms.u_fade.value = 0.9999; // Reset efek fading saat bunga muncul
        pointer.clicked = false;
    }

    shaderMaterial.uniforms.u_stop_time.value += clock.getDelta();
    shaderMaterial.uniforms.u_fade.value *= 0.9999; // Perlahan menghilang setiap frame
